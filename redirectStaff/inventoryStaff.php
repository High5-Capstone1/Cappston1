        ...
        if ($inv['quantity'] <= $inv['low_stock_level']) {
            ...
            if ($stmt->get_result()->num_rows === 0) {
                ...
                $stmt = $conn->prepare("
                    ...
                    VALUES (?, ?, ?, 'pending', ?, NOW())
                ");
                $stmt->bind_param("iiii", $store_id, $item_id, $requested_qty, $staff_id);
                $stmt->execute();
...
");
$stmt->bind_param("iiiis", $staff_id, $store_id, $new_request_id, $item_name, $requested_qty);
$stmt->execute();

                $_SESSION['staff_request_success'] = "Stock request sent successfully.";

              $base = dirname($_SERVER['PHP_SELF']);
                header("Location: " . $base . "/inventoryStaff.php");
                exit();
            } else {
...
<head>
    ...
    <title>Staff Inventory – Store #<?= htmlspecialchars($store_id) ?></title>
                ...
                <td>
                    ...
                    <form method="POST" id="form_<?= $row['item_id'] ?>" style="margin:0;">
                        ...
                        <div class="request-form-cell">
                            <input type="number" name="requested_qty" min="1" required placeholder="Qty"
                                   class="qty-input">
...
<?php if (isset($_SESSION['staff_request_success'])): ?>
<script>
document.addEventListener('DOMContentLoaded', function () {
    Swal.fire({
        icon: "success",
        title: "Success",
        text: "<?= addslashes($_SESSION['staff_request_success']); ?>",
...
<?php unset($_SESSION['staff_request_success']); ?>
<?php endif; ?>
